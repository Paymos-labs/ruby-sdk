# AGENTS.md — Paymos Ruby SDK

Client for the Paymos Merchant API. One of eight language SDKs implementing the same wire
contract; the shared golden vectors in `conformance/` are the arbiter when behaviour is in doubt.
...
- Do not change wire behaviour without updating `conformance/` — those vectors are shared across all eight SDKs and are verified byte-for-byte against each other.
- Do not "fix" a signature mismatch by loosening verification. The canonical strings above are exact, including the leading `?` on the query and the empty-string body hash.
